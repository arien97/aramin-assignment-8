...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances):
        ...
        # Fit logistic regression model and record parameters
        ...
        xx_meshgrid, yy_meshgrid = np.meshgrid(np.linspace(x_min, x_max, 200), np.linspace(y_min, y_max, 200))
        Z = model.predict_proba(np.c_[xx_meshgrid.ravel(), yy_meshgrid.ravel()])[:, 1]
        Z = Z.reshape(xx_meshgrid.shape)
        
        contour_levels = [0.7]
        class_1_contour = plt.contour(xx_meshgrid, yy_meshgrid, Z, levels=contour_levels, colors=['red'])
        class_0_contour = plt.contour(xx_meshgrid, yy_meshgrid, Z, levels=[1 - level for level in contour_levels], colors=['blue'])
